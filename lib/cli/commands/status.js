...
function resolvePlanPath(planArg) {
  ...
  const inPlans = path.join(getPlansDir(), planArg);
  if (fs.existsSync(inPlans)) return inPlans;
  const inCompleted = path.join(getCompletedDir(), planArg);
  if (fs.existsSync(inCompleted)) return inCompleted;
  return inPlans; // default so caller's error message still works
}
