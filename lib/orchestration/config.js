...
function getAgentPriority() {
  ...
  if (envPriority && envPriority.trim()) {
    return envPriority.trim().split(',').map(s => s.trim());
  }
...
function getInstalledAgentPriority() {
  ...
  const filteredAgents = requestedAgents.filter(agent => installedAgents.includes(agent));

  // Warn about agents in priority that aren't installed
  const skippedAgents = requestedAgents.filter(agent => !installedAgents.includes(agent));
  if (skippedAgents.length > 0) {
    const missingDirs = skippedAgents.map(a => `~/.${a}`).join(', ');
    console.warn(`[config] Skipping unconfigured agents: ${skippedAgents.join(', ')} (missing: ${missingDirs})`);
  }

  if (filteredAgents.length === 0) {
    console.warn(`[config] No configured agents found. Install an agent CLI and create its config directory (e.g., ~/.claude, ~/.codex, ~/.gemini)`);
  }
