...
class ProgressTracker {
  ...
  /**
   * Log when parallel steps start as a batch
   * @param {string[]} stepIds - Array of step IDs starting in parallel
   */
  logParallelStepStart(stepIds) {
    if (this.isFirstStep) {
      this.isFirstStep = false;
    } else {
      console.log("");
    ...
    const endNum = processed + stepIds.length;

    console.log(
      `[Progress] Starting ${stepList} in parallel (${startNum}-${endNum} of ${this.totalSteps})`
    );
  }
