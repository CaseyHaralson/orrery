...
async function orchestrate(options = {}) {
  ...
  for (const planFile of planFiles) {
    ...
    if (isComplete && isSuccessful) {
      ...
      console.log("  2. Run 'orrery unblock --all' to reset");
      console.log("  3. Run 'orrery exec --resume' to continue");
...
async function processPlan(planFile, completedDir, reportsDir) {
  ...
  const tracker = new ProgressTracker(plan.steps.length, path.basename(planFile));
...
async function startSteps(planFile, stepIds, activeAgents, tracker) {
  ...
  const handle = invokeAgentWithFailover(config, tempPlanFile, stepIds, REPO_ROOT, {
    onStdout: (text, ids) => {
 ...
 */
async function waitForAgentCompletion(planFile, activeAgents, reportsDir, tracker) {
