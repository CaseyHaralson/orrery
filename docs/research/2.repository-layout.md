...
### Canonical Directories:
  ...
  - agent/schemas/ – YAML-formatted schemas defining structured formats used by the agents to communicate. Specifically:

    - plan-schema.yaml: defines the schema for the plan format (JSON Schema in YAML format) used in agent-to-agent handoff (task list, dependencies, etc).

    - report-schema.yaml: defines the schema for an execution report (JSON Schema in YAML format) produced after the Execute/Verify phases. Using explicit schemas helps validate that plans and reports are well-formed and allows different agents or tools to parse them consistently.
